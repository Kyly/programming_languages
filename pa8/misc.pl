...
available_at(X,Y) :- taqueria(Y,_,L),
					 isin(X,L).

multi_available_helper(X) :- available_at(X,Y),
                      available_at(X,Z),
                      Y \= Z. 
multi_available(X) :- bagof(Y, multi_available_helper(Y), L),
                      remove_duplicates(L, L1),
                      isin(X,L1).

overworked_helper(X) :- taqueria(Y,L1,_),
                        isin(X,L1),
                        taqueria(Z,L2,_),
                        isin(X,L2),
                        Y \= Z.
overworked(X) :- bagof(Y, overworked_helper(Y), L),
                 remove_duplicates(L, L1),
                 isin(X, L1). 

list_cost([],K) :- K is 0.
list_cost([I|T],K) :- cost(I,C1),
                    list_cost(T,C2),
                    K is C1 + C2. 
total_cost(X,K) :- ingredients(X,L),
                   list_cost(L,K).

list_ingredients([],L).
list_ingredients([H|T],L) :- isin(H, L),
                             list_ingredients(T,L).
has_ingredients(X,L) :- ingredients(X,L2),
                        list_ingredients(L,L2).

avoids_i([],L).
avoids_i([H|T],L) :- \+isin(H,L),
                     avoids_i(T,L).
avoids_ingredients(X,L) :- ingredients(X,L2),
                           avoids_i(L,L2). 
