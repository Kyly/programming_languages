...
let rec build (rand,depth) =
	if depth = 0 then 
		...
		match rand (0,20) with
		| 0 -> if rand (0,1) = 1 then buildX() else buildY()
		| _ -> match rand (0,12) with
			| 0 -> buildThresh (build (rand, depth), build (rand, depth), build (rand, depth), build (rand, depth))
...
(* build2: (int*int->int) * int -> Expr
   Builds and expression tree. The second argument is the depth,
   the first is a random function.

   build2 adds on to build my making use buildExpNegAbs and
   buildLosPlus
*)
...
let rec build2 (rand,depth) =
	if depth = 0 then 
		...
		| 0 -> if rand (0,1) = 1 then buildX() else buildY()
		| _ -> match rand (0,12) with
			| 0 -> buildThresh (build2 (rand, depth), build2 (rand, depth), build2 (rand, depth), build2 (rand, depth))
			| _ -> match rand (1, 7) with
			| 1 -> buildSine(build2 (rand, depth))
			| 2 -> buildCosine(build2 (rand, depth))
			| 3 -> buildAverage (build2 (rand, depth), build2 (rand, depth))
			| 4 -> buildTimes (build2 (rand, depth), build2 (rand, depth))
			| 5 -> buildExpNegAbs(build2 (rand, depth))
			| _ -> buildLogPlus(build2 (rand, depth), build2 (rand, depth), build2 (rand, depth))
