...
def load_passwd(filename):
    ...
    for line in lines:
        u = line.split(":")
        users.append({'account':u[0],'password':u[1],'UID':int(u[2]),'GID':int(u[3]),'GECOS':u[4],'directory':u[5],'shell':u[6].rstrip("\n")})
    return users
...
def transform_reverse_caps(w):
    ...
    return [transform_reverse(tc)[1] for tc in transform_capitalize(w)]
